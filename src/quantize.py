...
from src.GPTQ import GenericGPTQRunner, InputRecorder
from src.model import find_multiple
...
def get_group_qparams(w, n_bit=4, groupsize=128):
    ...
    return scales.to(torch.float32).reshape(w.shape[0], -1), zeros.to(
        torch.float32
    ).reshape(w.shape[0], -1)
...
def pack_scales_and_zeros(scales, zeros):
    ...
    assert zeros.dtype == torch.float32
    return (
        torch.cat(
            [
...
class GPTQQuantHandler(QuantHandler):
    ...
    def get_inputs(
        ...
        logger.info("Obtaining GPTQ calibration inputs on: ", calibration_tasks)
...
def prepare_int4_weight_and_scales_and_zeros(weight_bf16, groupsize, inner_k_tiles):
    logger.info(weight_bf16)
    logger.info(groupsize)
    logger.info(inner_k_tiles)
    
    weight_bf16 = weight_bf16.to("mps")
    weight_int32, scales_and_zeros = group_quantize_tensor(
        weight_bf16, n_bit=4, groupsize=groupsize
    )
    weight_int32 = weight_int32.to("mps")
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                ...
                if not _check_linear_int4_k(
                    ...
                    if self.padding:
                        logger.warning(
                            f"{fqn} is padded to satisfy in_features % 1024 == 0"
                        )
                        ...
                        logger.warning(
                            f"{fqn} is skipped, int4 requires that in_features is 32, 64, or is divisible by 1024, and that groupsize and inner_k_tiles*16 evenly divide into it"
                        )
                        continue
                (
                    weight_int4pack,
                    ...
                    weight.to(torch.float32).to("mps"),
                    self.groupsize,
                    self.inner_k_tiles,
                )
...
class WeightOnlyInt4Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "scales_and_zeros",
            torch.empty(
                (in_features // groupsize, out_features, 2), dtype=torch.float32
            ),
        )
